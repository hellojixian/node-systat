...
//normalized temp data
module.exports.getCPUTemperatures = function()
{
    var results = systat.getCPUTemperatures();
    if(!Array.isArray(results)) return [];    
    for (var i=0;i<results.length;i++){
        results[i] = parseInt(parseInt(results[i]).toString().substring(0,2));
    }
    return results;
}

//normalized temp data
module.exports.getSystemTemperatures = function()
{
    var results = systat.getSystemTemperatures();
    if(!Array.isArray(results)) return [];    
    for (var i=0;i<results.length;i++){
        results[i] = parseInt(parseInt(results[i]).toString().substring(0,2));
    }
